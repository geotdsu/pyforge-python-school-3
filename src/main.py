...
tags_metadata = [
    ...
    {"name": "Molecule Management", "description": "Crud operations"},
    ...
    **Exceptions:**
    ...
    if molecule.molecule_id in molecules:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"Molecule with the ID {molecule.molecule_id} already exists")
    ...
    if molecule_object is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid SMILES string")
...
def substructure_search(query: schemas.SubstructureQuery):
    ...
    **Exceptions:**
    ...
    if not query.substructure:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Substructure query cannot be empty.")
...
async def upload_file(file: UploadFile = File(...)):
    ...
    **Exceptions:**
    ...
    if file.filename == "":
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="No file selected for upload")
    ...
    try:
        ...
        if not any(header in reader.fieldnames for header in valid_headers):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail="CSV file must contain 'SMILES', 'smiles', or 'Smiles' header.")
        ...
        for index, row in enumerate(reader, start=len(molecules) + 1):
            ...
            smiles_value = row.get("SMILES") or row.get(
                "smiles") or row.get("Smiles")

            # Validate the SMILES value
            if not smiles_value:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                    detail=f"Missing 'SMILES' or 'smiles' or 'Smiles' value in row {index}")

            # Validate the SMILES string using RDKit
            molecule_object = Chem.MolFromSmiles(smiles_value)
            if molecule_object is None:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                    detail=f"Invalid SMILES string in row {index}: {smiles_value}")
