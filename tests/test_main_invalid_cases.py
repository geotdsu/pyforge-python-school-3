...
def test_add_invalid_smiles_syntax(molecule_id, smiles_string, expected_status, expected_detail):
    response = client.post(
        "/add", json={"molecule_id": molecule_id, "smiles": smiles_string})
...
def test_add_duplicate_molecule_id():
    ...
    assert response.json() == {
        "detail": "Molecule with the ID 1 already exists"}
...
@pytest.mark.parametrize("molecule_id,smiles,expected_status,expected_detail", [
    # Invalid SMILES syntax
    (1, "Protein", 400, "Invalid SMILES string"),
    ...
    # Non-existing molecule ID
    (999, "CCO", 404, "Molecule with the ID 999 is not found")
])
...
@pytest.mark.parametrize("file_path, expected_status, expected_detail", [
    ...
    ("missing_smiles_column.csv", 400,
     "CSV file must contain 'SMILES', 'smiles', or 'Smiles' header."),
    ("missing_smiles_row.csv", 400,
     "Missing 'SMILES' or 'smiles' or 'Smiles' value in row 3"),
